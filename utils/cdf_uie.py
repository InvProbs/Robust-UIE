...
def to_ssim_skimage(enhance, gt):
    ...
    enhance_list_np = [enhance_list[ind].permute(0, 2, 3, 1).data.cpu().numpy().squeeze() for ind in
                       range(len(enhance_list))]
    gt_list_np = [gt_list[ind].permute(0, 2, 3, 1).data.cpu().numpy().squeeze() for ind in range(len(enhance_list))]
    ssim_list = [compare_ssim(enhance_list_np[ind], gt_list_np[ind], data_range=1, multichannel=True) for ind in
                 range(len(enhance_list))]
