""" code borrowed from https://github.com/CXH-Research/UIE-UnFold/blob/main/models/model.py """
...
class UNetUpBlock(nn.Module):
    def __init__(self, in_size, out_size, relu_slope, output_pad):
        super(UNetUpBlock, self).__init__()
        self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=2, stride=2, output_padding=(output_pad, 0),
                                     bias=True)
        self.conv_block = UNetConvBlock(out_size * 2, out_size, False, relu_slope)

    def forward(self, x, bridge):
        up = self.up(x)  # [4, 100, 67, 96]
...
class Decoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=5):
        ...
        for i in range(depth - 1):
            output_pad = 1 if i <= 1 else 0
            self.body.append(UNetUpBlock(in_size=n_feat + scale_unetfeats * (depth - i - 1), output_pad=output_pad,
                                         out_size=n_feat + scale_unetfeats * (depth - i - 2), relu_slope=0.2))
            self.skip_conv.append(
                nn.Conv2d(n_feat + scale_unetfeats * (depth - i - 1), n_feat + scale_unetfeats * (depth - i - 2), 3,
                          1,
                          1))
    ...
    def forward(self, x, bridges):
        ...
        for i, up in enumerate(self.body):
            x = up(x, self.skip_conv[i](bridges[-i - 1]))  # [4, 100, 33, 48], [4, 80, 67, 96]
            res.append(x)
            # break
...
class Model(nn.Module):
    def __init__(self, args, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8,
                 kernel_size=3, reduction=4, bias=False, depth=5):
        ...
        """ new blocks """
        self.netA = netA_UIEB()
        self.netAT = PhyNNadjoint()
        self.eta = nn.Parameter(torch.Tensor([-1]))
        self.maxiters = args.maxiters

        self.shallow_feat = nn.Sequential(conv(3, n_feat, kernel_size, bias=bias),
                                          CAB(n_feat, kernel_size, reduction, bias=bias, act=act))
        self.encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=4, csff=False)
        self.decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=4)
        self.sam = SAM(n_feat, kernel_size=1, bias=bias)
        self.merge = mergeblock(n_feat, 3, True)

    def forward(self, y):
        xk = torch.clone(y)
        bs, c, H, W = xk.shape
        samfeatures = torch.zeros([bs, 40, H, W]).to(xk.device)
        for i in range(self.maxiters):
            dk = xk - torch.exp(self.eta) * self.netAT(self.netA(xk) - y)
            shlw = self.shallow_feat(dk)
            shlw_cat = self.merge(shlw, samfeatures)
            featk, featk_fin = self.encoder(shlw_cat)
            res = self.decoder(featk_fin, featk)
            samfeatures, xk = self.sam(res[-1], dk)
        return xk

""" netA and netAT """


class netA_UIEB(nn.Module):
    def __init__(self, nlayers=5):
        super(netA_UIEB, self).__init__()
        self.beta = nn.Parameter(torch.ones(1))
        self.B = nn.Parameter(torch.ones(1, 3, 1, 1))
        ...
        self.f = DnCNN(nlayers=4, features=8)
    ...
    def forward(self, xk):
        yk = xk * self.T(xk) + self.B * (torch.ones_like(xk).to('cuda') - self.T(xk)) + self.f(xk)
        return yk
...
class Tblock(nn.Module):
    def __init__(self):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(6, 8, 3, padding=1, dilation=1),  # 66 x 88 -> 66 x 88
            nn.PReLU(),
            nn.Conv2d(8, 16, 3, padding=1, dilation=1),  #
            nn.PReLU(),
            nn.Conv2d(16, 16, 3, padding=2, dilation=2),  #
            nn.PReLU(),
            nn.Conv2d(16, 16, 3, padding=5, dilation=5),
            nn.PReLU(),
            nn.Conv2d(16, 16, 3, padding=2, dilation=2),  #
            nn.PReLU(),
...
class PhyNNadjoint(nn.Module):
    def __init__(self):
        super(PhyNNadjoint, self).__init__()

        self.BNet = Bblock()
        self.TNet = Tblock()
        self.postNet = DnCNN(nlayers=4, features=8)

    def forward(self, y):
        B = self.BNet(y)
        T = self.TNet(torch.cat((y * 0 + B, y), 1))
        out = ((y - B) * T + B)
        return self.postNet(out)


class DnCNN(nn.Module):
    def __init__(self, nlayers=9, features=8):
        super(DnCNN, self).__init__()
        layers = [nn.Conv2d(in_channels=3, out_channels=features, kernel_size=3, padding=1, bias=True),
                  nn.PReLU()]
        for _ in range(nlayers - 2):
            layers.append(nn.Conv2d(in_channels=features, out_channels=features, kernel_size=3, padding=1, bias=True))
            layers.append(nn.BatchNorm2d(features))
            layers.append(nn.PReLU())
        layers.append(nn.Conv2d(in_channels=features, out_channels=3, kernel_size=3, padding=1, bias=True))
...
class PhyNN_fixpoint(nn.Module):
    def __init__(self, args, n_feat=40, scale_unetfeats=20, kernel_size=3, reduction=4, bias=False):
        super(PhyNN_fixpoint, self).__init__()
        act = nn.PReLU()
        self.BNet = Bblock()
        self.TNet = Tblock()
        self.Res = convNet(nlayers=5, features=16)
        self.maxiters = args.maxiters
        ...
        self.encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=4, csff=False)
        self.decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=4)
        self.sam = SAM2(n_feat, kernel_size=1, bias=bias)
        self.merge = mergeblock(n_feat, 3, True)

    def A_inv(self, xk, yk, samfeatures):
        B = self.BNet(yk)

        shlw = self.shallow_feat(xk)
        shlw_cat = self.merge(shlw, samfeatures)
        featk, featk_fin = self.encoder(shlw_cat)
        res = self.decoder(featk_fin, featk)
        samfeatures, T = self.sam(res[-1], yk)

        out = ((yk - B) * T + B)
        return out, samfeatures
    ...
    def forward(self, y):
        ...
        self.xk_list = []
        for i in range(self.maxiters):
            xk, samfeatures = self.A_inv(xk, y - self.Res(xk), samfeatures)
